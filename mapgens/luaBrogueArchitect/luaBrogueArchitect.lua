...
--  that the code currently relies on a few external functions: tilemap_*() functions,
--  a preinitialized rng object with a random() method; as well as the rotLove library 
--  for cellular automata support (critical for the frequently used 'cavern' type room 
--  generator).
--
--  Note of caution:
...
function mapgen_broguestyle_room_cavern_create_blob_helper(tilemap, iterations, min_width, min_height, max_width, max_height, percent_seeded)
	...
	while attempt <= max_attempts do
		...
		-- DEBUG
		tilemap_show(mapgen_broguestyle_room_cavern_callback_tilemap,"RAW BLOBS")
...
function mapgen_broguestyle_room_cavern(tilemap,minwidth,minheight,maxwidth,maxheight)

	-- validate input
	print("mapgen_broguestyle_room_cavern() supplied " .. #tilemap .. "x" .. #tilemap[1] .. " tilemap, width=" .. minwidth .. "-" .. maxwidth .. ", height=" .. minheight .. "-" .. maxheight)
	local iterations = 6
